/* kernel.c - the C part of the kernel - MODIFIED AS A KERNEL LOADER */
...
void lmain(unsigned long magic, unsigned long addr)
{
    ...
    // loading our kernel, which is the first module
    if (mbi->mods_count > 1)
    {
        multiboot_module_t *mod = (multiboot_module_t *)(mbi->mods_addr);
        // void *kernel_entry = load_elf_module(mod->mod_start, mod->mod_end);
    }
}
...
void print_mbi_info(multiboot_info_t *mbi)
{
    /* Print out the flags. */
    printf("flags = 0x%x\n", (unsigned)mbi->flags);
